/**
 * SHA-256 over bytes, written out so the message-hash check before a wallet
 * signature does not depend on `crypto.subtle` being available (it is
 * missing in some test and embedded-browser environments) and never on a
 * backend package the app may not import.
 */
...
export function sha256(message: Uint8Array): Uint8Array {
  ...
  for (let i = 0; i < 8; i += 1) {
    outView.setUint32(i * 4, h[i] as number);
  }
  return out;
}

export function sha256Hex(message: Uint8Array): string {
  return Array.from(sha256(message), (byte) => byte.toString(16).padStart(2, '0')).join('');
}
