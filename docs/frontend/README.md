...
delivered Explore's Stocks tab (`tab=instruments`) over real
backend-admitted instruments, the exact-id market detail page and
account-scoped watchlists; sessions **F06** to **F08** delivered sourced
research, the basket builder and publishing with version history; session
**F09** delivered the one review screen for basket investments and single
buys over the B09 execution plans (every term, bound to the plan hash,
approved before any wallet is asked to sign); session **F10** delivered
wallet signing, the execution timeline with recovery and receipts; session
**F11** the portfolio with reconciled holdings, performance and the
receipt explanation; session **F12** the strategy explorer, rankings,
creator pages and explicit acceptance of a creator's newer version;
session **D01** the documentation site (`apps/docs`). F13 to F20 are not
built; the production completion plan carries them (see the last row of
the feature map).

Hosting (2026-09-25): the app runs on Vercel at
https://markov-web-theta.vercel.app in staging mode (`MARKOV_ENV=staging`)
against a placeholder `MARKOV_API_ORIGIN`, so it shows "Backend
unreachable"; the documentation site runs on Vercel at
https://markov-docs.vercel.app/docs/ and is also proxied at `/docs` of the
app deployment. Both were built from commit `7840da2` before the history
rewrite (its rewritten equivalent `9b6bde3` has the identical tree). No
backend is hosted anywhere (Railway is selected for the backend services,
not provisioned), no program is deployed on any cluster and nothing
financial is live. `markov.pet` is not routed to these deployments (see
`docs/markov/operations.md`, "Frontends on Vercel").
...
navigation, environment chip; F12: the Explore Strategies tab; the top-bar
link row and the Set Rules right rail remain for the shell pass, F19).
...
pnpm web:typecheck               # formatters, api-client, ui, shell and web
...
MARKOV_TEST_DATABASE_URL=postgres://markov:markov@127.0.0.1:5432/markov_test pnpm web:e2e   # also every journey that needs the API (auth, wallets, markets, research, builder, publishing, review, execution, portfolio, discovery) against the local API in test mode (fixture RPC on 3901); without it those specs are skipped
...
| Area                                  | Session | Readiness (after F12) |
| ------------------------------------- | ------- | --------------------- |
...
| Execution `/activity/[intentId]` and the panel on the review: the API builds each transaction of the approved plan and the panel shows it before the wallet opens; signing preconditions checked in the browser (intent, plan hash, signer, cluster, capability, expiry, message hash, simulation) and again by the API; one `solana:signTransaction` request with the returned bytes checked against the prepared message; one submission; wallet decline, silence, mutation, account and session changes and lost answers handled without a second intent or transaction; per-transaction timeline with stages, fills, fees paid, signatures, explorer links and reconciliation evidence read on a bounded schedule and announced accessibly; cancel and cancellation requests with consequences; reviewed completion of a partially completed basket; `/activity` with URL filters and stable deep links; receipts issued and read at `/receipts/[receiptId]` with public opt-in | F10 | IMPLEMENTED, FIXTURE_VERIFIED (jsdom), e2e against the local B10 to B12 API with the fixture chain, fixture venue and fixture wallet; no live venue or cluster (OD-21); receipts signed with a local key (OD-22); cryptographic verification of receipts is offline (`markov receipts verify`), and F11 did not add it in the browser |
| Design reference realignment: navigation (Explore, Build, Portfolio, Activity; eyes and wordmark lead home), environment chip, periwinkle accent with blue eyes, cream bezel and near-black screen, measured-contrast table | F10 | IMPLEMENTED, FIXTURE_VERIFIED (token contrast test); the Explore Strategies table was built to the reference in F12; the single-row top-bar link layout and the Set Rules right rail remain for the shell pass (F19) |
| Portfolio `/portfolio` and `/portfolio/[instanceId]`, receipt explanation: holdings against the last chain observation with multiplier-aware quantities and the API's valuations, reconciliation on request, external flows explained by the owner at wallet level, strategy instances (created by the review before a basket intent), target against actual allocation with drift among the invested legs, FIFO lots with cost and fees, personal against model performance over one period with methodology labels, accessible chart summaries, exact figures and JSON downloads, journal history; receipts show requested against filled and the fee cap | F11 | IMPLEMENTED, FIXTURE_VERIFIED (jsdom with independently expected sums), e2e against the local API (test mode) and the fixture chain |
| Discovery `/explore` (Strategies tab), `/rankings`, `/creators/[publisherWallet]`, provenance and model performance on public strategy and version pages, proposal acceptance on an instance: registered strategies with chain provenance and the model ranking's own entry (a young or incomplete recipe listed without a rank and without a number, with the reason), filters and stable pages in the URL, one ranking cohort per period beside the methodology, creator pages from chain records, Following composed from the private list and never cached with public rows, a creator's newer version shown as a diff and moved to only by explicit acceptance | F12 | IMPLEMENTED, FIXTURE_VERIFIED (jsdom, proxy), e2e against the local API (test mode) with the fixture ledger; no live cluster or real follower population |
| Recurring and rebalance proposals (F13, over B16), companion panel and typed proposals (F14, over B15), voice (F15), account, session and device settings (F16), Tessera (F17), Meteora observation and DBC simulation (F18), operational status (F19), browser restore (F20) | F13 to F20 | not built; the production completion plan carries them as P14, P16, E01, P17, E02, E04, P18 and P19 in that order; the B15 and B16 backends they build on exist |
...
nothing to the backend. F10 uses the B10 transaction, submission,
execution and reconciliation routes (`/v1/me/intents/{intentId}/transactions`,
`…/transactions/{transactionIndex}/submissions`, `…/execution`,
`…/execution/reconciliations`), the B11 continuation
(`continuationOfIntentId`) and the B12 receipt routes (`…/receipts`,
`/v1/receipts/{receiptId}`, `/v1/receipts/keys`,
`/v1/me/receipts/{receiptId}/visibility`); F11 uses the B07 instance
routes (`/v1/me/instances`), the B12 accounting routes (holdings,
reconciliations, journal, acknowledgements) and the B13 performance routes
and methodology; F12 uses the B14 discovery routes (`/v1/strategies`,
`/v1/creators/{publisherWallet}`), the B13 model ranking
(`/v1/rankings/model`) and the pin (`/v1/me/instances/{instanceId}/pin`,
open to non-owners since B14). F10 to F12 added nothing to the backend.
The app never reads provider
