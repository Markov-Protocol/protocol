...
## F12 — strategy exploration, following and honest rankings (2026-09-25)

Environment as for F11: the real API in test mode against the fixture chain
and the fixture registry ledger (`scripts/dev/web-e2e-api.sh`), the fixture
wallet signing real registration bytes, the reference prices the catalog
ingestion recorded from the fixture feeds. The explorer, the ranking and
the creator pages are the B14 and B13 routes as published; no backend
change was needed. No live price source, venue, cluster or real wallet was
involved, and nothing was ranked from an account.

| Check | Command | Result |
| ----- | ------- | ------ |
| Backend additions | none | the app consumes `GET /v1/strategies`, `GET /v1/creators/{wallet}`, `GET /v1/rankings/model` and `POST /v1/me/instances/{id}/pin` as B14 and B13 published them |
| App-owned API proxy | `pnpm exec vitest run --project node apps/web/test/server/proxy.test.ts` | 18 tests: the F12 block allows the three public discovery reads with bounded queries and the explicit pin, refuses a creator path that is not a wallet address and every moderation route; the F06, F08 and F11 blocks now assert the pin is allowed and moderation refused |
| Contract matrix | `pnpm exec vitest run --project node packages/api-client/test/compatibility.test.ts` | 83 tests: the four F12 operations exist in the frozen OpenAPI document with the statuses the app handles |
| Pure discovery model | `pnpm exec vitest run --project web apps/web/test/discovery-model.test.ts` | 6 tests against hand-written expectations: a fraction of 0.035 reads +3.50% and 0.0125 as a -1.25% drawdown only for a ranked entry, a rank without a return stays unranked, every ineligibility reason in the methodology's words, universe and issuer labels that claim nothing, allocation and completeness as counts, followed ids only from the private list, the proposal diff with the backend's one-sided turnover (1,000 bps for a 1,000 bps leg move plus 1,000 bps of cash), the source line with the read time |
| Discovery screens | `pnpm exec vitest run --project web apps/web/test/discovery.test.tsx` | 8 tests: ranked and young rows from one public read (`/v1/strategies?limit=25`, no follows read while signed out, no Following badge), the Following badge and section composed from the private list with the filters in the URL, a rate limit, contract drift and an expired page as failures (never an empty list), one ranking cohort with the methodology beside it and a reason per unranked entry, the creator page and its honest not-found, the creator link and ranking line on a public strategy page, the proposal diff with an explicit acceptance posting exactly `{versionId}`, and a proposed version that is no longer public shown as unacceptable with the button disabled |
| Explore, publishing and portfolio regressions | `pnpm exec vitest run --project web apps/web/test/markets.test.tsx apps/web/test/publishing.test.tsx apps/web/test/portfolio.test.tsx` | 35 tests: Explore lands on Strategies by default and the Stocks tab keeps its URL value (`tab=instruments`) |
| Whole web project | `pnpm exec vitest run --project web` | 28 files, 213 tests |
| Browser journey | `cd apps/web && MARKOV_TEST_DATABASE_URL=… pnpm exec playwright test e2e/discovery.spec.ts` | 2 passed (46.8 s) on the desktop and phone profiles, each with its own strategy title: the creator registers version 1 with the fixture wallet; an anonymous visitor sees it on Explore (Strategies selected by default) without a rank, with the reason and no percentage, with 0 followers, the creator wallet linked and the source line naming stocks-v1; search narrows and lives in the URL; the rankings page lists the same entry unranked beside the methodology panel; the creator page shows the facts and the row; the strategy page carries the creator link and the ranking line and the version page the model performance panel; a second person follows (button, follower count 1, Following badge and section), pins version 1 in an instance of their own (the review's instance call), sees no proposal; the creator edits to 50/30/20, freezes and registers version 2; the follower's instance shows the proposal with `FXAERO: 60.00% → 50.00%`, `Cash: 10.00% → 20.00%` and a 10.00% turnover, accepts, and is pinned to version 2 after a reload as well |
| Full gate | `pnpm verify` | green: lint (Biome, 604 files, the 3 pre-existing warnings and 3 infos), typecheck across the workspace and the web app, 87 test files / 640 tests, dependency boundaries (31 workspace packages), OpenAPI document current, API client types current, migrations current, tokens.css current, the web production build (with `/rankings` and `/creators/[publisherWallet]` as dynamic routes) and the docs site build (69 synced pages, 139 operations in 14 tag pages, 21 command groups) |

Evidence: `docs/frontend/evidence/F12/` (the Explore Strategies tab with
the unranked recipe and its reason, the rankings cohort with the
...
| Public caches never include per-user followed state or private portfolio data | The explorer, ranking and creator queries are keyed by filters only and the API rows have no field for a reader; followed state comes from the private follows query, is composed at render time and is never requested while signed out (jsdom asserts the calls); public rows contain no user id, wallet id other than the publisher, instance id or draft (B14 API test) | complete (F12) |
| A creator update displays a diff and never trades automatically | The proposal panel shows legs added, removed and changed, the cash move and the one-sided turnover computed from the two public payloads, states that nothing is bought or sold, and moves the pin only on the explicit `POST …/pin` (jsdom and the F12 journey); the API offers a version to followers only once it is registered and moves no pin itself (B14) | complete (F12) |
